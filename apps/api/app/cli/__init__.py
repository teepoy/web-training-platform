...
def main() -> None:
    if len(sys.argv) > 1 and sys.argv[1] == "deployments":
        from app.cli.deployments import deployments_group

        sys.argv = sys.argv[:1] + sys.argv[2:]  # strip "deployments" for Click
        deployments_group()
        return

    parser = argparse.ArgumentParser(
        description="Platform CLI (see also: ftapi deployments --help)",
        prog="ftapi",
    )
