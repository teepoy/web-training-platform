...
async def export_parquet(
    ...
    for sample in samples:
        image_structs = [
            _build_image_struct(uri) for uri in sample.image_uris
        ]
    ...
    metadata_str_array = pa.array(
        [str(m) for m in metadata_arrays], type=pa.string()
    )
    ...
    uri = await c.artifact_storage().put_bytes(
        f"exports/{dataset_id}/{filename}", parquet_bytes,
