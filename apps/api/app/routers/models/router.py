...
async def list_models(
    ...
    c = get_container()
    models = await c.model_service().list_models(
        org_id=org.id,
...
async def get_model(
    ...
    c = get_container()
    model = await c.model_service().get_model(model_id, org_id=org.id)
...
async def delete_model(
    ...
    c = get_container()
    await c.model_service().delete_model(model_id, org_id=org.id)
...
async def download_model(
    ...
    c = get_container()
    data, filename = await c.model_service().download_model(model_id, org_id=org.id)
    return Response(
        content=data,
...
async def upload_model(
    ...
    c = get_container()
    model = await c.model_service().upload_model(
        file=file,
        org_id=org.id,
