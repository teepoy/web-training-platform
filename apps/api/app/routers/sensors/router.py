...
async def list_sensors(
    ...
    return [
        ...
        for sensor in c.sensor_registry().list_all()
    ]
...
async def list_sensor_subscriptions(
    ...
    _get_sensor_or_404(c, sensor_id)
    subscriptions = await c.sensor_repository().list_subscriptions(sensor_id)
    return [
...
async def update_sensor_subscription(
    ...
    _get_sensor_or_404(c, sensor_id)
    existing = await c.sensor_repository().get_subscription(sub_id)
    ...
    subscription = await c.sensor_repository().update_subscription(
        sub_id,
        filter_config=payload.filter_config,
        enabled=payload.enabled,
    )
...
async def delete_sensor_subscription(
    ...
    if existing is None or existing.sensor_id != sensor_id:
        raise HTTPException(status_code=404, detail="sensor subscription not found")
    deleted = await c.sensor_repository().delete_subscription(sub_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="sensor subscription not found")
    return {"deleted": True}
...
async def ingest_sensor_events(
    ...
    _get_sensor_or_404(c, batch.sensor_id)
    summary = await c.sensor_dispatch().dispatch(batch.sensor_id, batch.events)
    if batch.watermark is not None:
        await c.sensor_repository().upsert_checkpoint(batch.sensor_id, batch.watermark)
...
def _get_sensor_or_404(c: Container, sensor_id: str) -> Any:
    sensor = c.sensor_registry().get(sensor_id)
    if sensor is None:
