...
@app.post("/api/v1/preview-sessions", response_model=PreviewSessionResponse)
async def create_preview_session(
    ...
    try:
        session = await container.preview_service().create_session(
...
@app.get("/api/v1/preview-sessions/{session_id}", response_model=PreviewSessionResponse)
...
async def start_preview_persist(
    ...
    status = await container.preview_service().start_persist(
        session_id=session_id,
        scope=scope,
        dataset_repo=container.repository(),
        label_studio_client=container.label_studio_client(),
    )
