"""Metadata schema inference for classify workspace prompts."""

from __future__ import annotations

from typing import Any

from app.modules.agent.classify.port.http.schemas import (
    DeclaredMetadataKey,
    MetadataKeyInfo,
)
...
def _scan_with_polars(
    ...
    for col_name in df.columns:
        ...
        if dtype in (
            ...
            min_val = col.min()
            max_val = col.max()
            info.min = min_val if isinstance(min_val, (int, float)) else None
            info.max = max_val if isinstance(max_val, (int, float)) else None
