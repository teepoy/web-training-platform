...
def resnet_sc_predictor(
    *,
    ...
    _ = lazyframe
    if materialized_dataset is None:
        raise ValueError("materialized_dataset is required for resnet50-sc-v1")
    ...
    for start in range(0, total, batch_size):
        ...
        for i in range(start, end):
            ...
            _pending_imgs.append(
                {
                    "_def_bytes": bytes(cast(bytes, def_bytes)),
                    "_ref_bytes": bytes(cast(bytes, ref_bytes)),
                    "_sid": sample_id,
                }
            )
