...
from app.modules.datasets.api.schemas import (
...
from app.modules.datasets.application.dataset_payload_store import DatasetPayloadStore
from app.modules.datasets.application.dataset_service import DatasetService
from app.modules.datasets.domain.dataset_payload import DatasetManifest
...
from app.modules.datasets.application.sparse_manifest import SparseManifestReader
...
async def create_dataset(
    ...
    if payload.storage_mode == DatasetStorageMode.FILE_SHARD_SPARSE:
        ...
        try:
            ...
            ls_client = c.label_studio_client()
    ...
    dataset = await c.repository().create_dataset(dataset)
    if payload.storage_mode == DatasetStorageMode.FILE_SHARD_SPARSE:
        ...
        await DatasetPayloadStore(c.artifact_storage()).put_manifest(
...
async def list_datasets(
    ...
    c = get_container()
    datasets = await c.repository().list_datasets(org_id=org.id)
...
async def get_sparse_summary(
    ...
    store = DatasetPayloadStore(c.artifact_storage())
    try:
...
async def delete_dataset(
    ...
    if dataset.storage_mode == DatasetStorageMode.FILE_SHARD_SPARSE:
        store = DatasetPayloadStore(storage=c.artifact_storage())
        await store.delete_dataset_payload(dataset_id, org_id=org.id)
    else:
        if dataset.ls_project_id:
            try:
                await c.label_studio_client().delete_project(int(dataset.ls_project_id))
    ...
    deleted = await c.repository().delete_dataset(dataset_id, org_id=org.id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Dataset not found")
...
async def update_label_space(
    ...
    updated = await c.repository().update_dataset_task_spec(dataset_id, new_task_spec)
    if updated is None:
        raise HTTPException(status_code=404, detail="Dataset not found")
...
async def set_dataset_public(
    ...
    ok = await c.repository().set_dataset_public(dataset_id, payload.is_public)
    if not ok:
        raise HTTPException(status_code=404, detail="dataset not found")
...
async def embed_sample(
    ...
    if uri.startswith("data:"):
        ...
        try:
            image_bytes = await c.artifact_storage().get_bytes(uri)
        except (FileNotFoundError, KeyError):
...
async def create_annotation(
    ...
    c = get_container()
    sample = await c.repository().get_sample(payload.sample_id)
    if sample is None:
        raise HTTPException(status_code=404, detail="sample not found")
    ...
    try:
        ...
        await c.label_studio_client().create_annotation(sample.ls_task_id, ls_result)
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(
    ...
    ann = await c.repository().create_annotation(ann)
    return ann
...
async def list_annotations_for_sample(
    ...
    c = get_container()
    if await c.repository().get_sample(sample_id) is None:
        raise HTTPException(status_code=404, detail="sample not found")
    return await c.repository().list_annotations_for_sample(sample_id)
...
async def update_annotation(
    ...
    c = get_container()
    result = await c.repository().update_annotation(annotation_id, payload.label)
    if result is None:
...
async def delete_annotation(
    ...
    c = get_container()
    deleted = await c.repository().delete_annotation(annotation_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="annotation not found")
    return Response(status_code=204)
...
async def bulk_create_annotations(
    ...
    for item in payload.annotations:
        ...
        await c.repository().create_annotation(ann)
        created += 1
...
async def export_dataset(
    ...
    c = get_container()
    dataset, samples, anns = await DatasetService(c).build_export_data(dataset_id)
    return c.artifacts().build_dataset_export(
        dataset=dataset,
        samples=samples,
        annotations=anns,
    )
...
async def export_dataset_persist(
    ...
    c = get_container()
    dataset, samples, anns = await DatasetService(c).build_export_data(dataset_id)
    uri = await c.artifacts().persist_dataset_export(
...
async def similarity_search(
    ...
    if sample is None or sample.dataset_id != dataset_id:
        raise HTTPException(status_code=404, detail="sample not found")
    return await c.feature_ops().similarity_search(
...
async def update_embed_config(
    ...
    await c.repository().update_dataset_embed_config(dataset_id, new_config)
...
async def download_artifact(
    ...
    c = get_container()
    artifact = await c.repository().get_artifact(artifact_id)
    if artifact is None:
        raise HTTPException(status_code=404, detail="artifact not found")
    try:
        data = await c.artifact_storage().get_bytes(artifact.uri)
