...
async def resnet_sc_train(ctx: TrainingRuntimeContext) -> RuntimeEventStream:
    ...
    async with open_sc_runtime_source(
        ...
        _runtime_logger().info(
            "ResNet SC trainer materializing input: rows=%d",
            selected_rows,
        )
        ...
        try:
            with tempfile.TemporaryDirectory(
                prefix=f"sc-resnet-training-{ctx.job_id}-"
                ...
                if len(labels) < 2:
                    raise RuntimeExecutionError(
                        ...
                        "SC ResNet training requires at least two labels after "
                ...
                if materialization.errors or skipped_samples:
                    yield RuntimeIssueReported(
                        ...
                        message="SC ResNet training skipped unusable image samples",
                        details={
...
async def resnet_sc_predictor(ctx: PredictionRuntimeContext) -> RuntimeEventStream:
    ...
    async with open_sc_runtime_source(
        ...
        try:
            with tempfile.TemporaryDirectory(
                prefix=f"sc-resnet-prediction-{ctx.job_id}-"
            ) as temporary_directory:
                ...
                if materialization.errors:
                    _runtime_logger().warning(
                        "ResNet prediction materialization completed with %d image errors",
                ...
                async def prediction_results():
                    for output in predict_resnet(checkpoint_path, samples):
    ...
    await repo.add_prediction_event(
        PredictionEvent(
            job_id=ctx.job_id,
            ts=datetime.now(UTC),
            message="SC ResNet prediction completed",
    ...
    if failed:
        yield RuntimeIssueReported(
            ...
            message="SC ResNet prediction completed with failed samples",
            details={"failed": failed, "processed": int(summary["processed"])},
        )
    yield OperationCompleted(summary)


__all__ = ["resnet_sc_predictor", "resnet_sc_train"]
