...
class TaskTrackerService:
    ...
    def __init__(
        ...
        *,
        ...
        self._config = config
        self._schedule_run_reader = schedule_run_reader
    ...
    async def list_tasks(
        ...
        *,
        ...
        if kind in (None, "training"):
            jobs, training_total = await self._training_repository.list_jobs_paginated(
                org_id=org_id,
        ...
        if kind in (None, "prediction"):
            (
                jobs,
                prediction_total,
            ) = await self._prediction_repository.list_prediction_jobs_paginated(
                org_id=org_id,
                offset=0 if kind is None else offset,
                limit=fetch_limit if kind is None else limit,
            )
        ...
        if kind in (None, "schedule_run"):
            schedule_tasks, schedule_total = await self._list_schedule_run_records(
                org_id,
                offset=0 if kind is None else offset,
                limit=fetch_limit if kind is None else limit,
            )
        ...
        dataset_names = await self._dataset_repository.list_dataset_names(
    ...
    async def cancel_task(self, task_id: str, org_id: str) -> bool:
        ...
        if task.task_kind == "training":
            ...
            await self._training_repository.update_job_status(
    ...
    async def _resolve_task(self, task_id: str, org_id: str) -> _TaskRecord | None:
        ...
        prediction = await self._prediction_repository.get_prediction_job(
            task_id, org_id=org_id
        )
        if prediction is not None:
            return self._to_prediction_record(prediction)
        try:
            run = await self._schedule_run_reader.get_run(task_id, org_id)
        except HTTPException as exc:
            if exc.status_code == 404:
                return None
            raise
        return self._to_schedule_run_record(run)

    async def _list_schedule_run_records(
        self,
        org_id: str,
        *,
        offset: int,
    ...
    def _to_schedule_run_record(
        ...
        created_at = (
            self._timestamp_sort_value(run, "start_time")
            or self._timestamp_sort_value(run, "expected_start_time")
            or run.get("schedule_created_at")
        )
        updated_at = (
            self._timestamp_sort_value(run, "end_time")
            or self._timestamp_sort_value(run, "start_time")
            or self._timestamp_sort_value(run, "expected_start_time")
            or created_at
        )
        ...
        schedule_id = str(run.get("schedule_id", ""))
        return _TaskRecord(
            ...
            platform_job=type(
                ...
                {
                    "id": run_id,
                    "created_by": str(run.get("created_by", "system")),
    ...
    async def _gpu_job_id(self, task: _TaskRecord) -> str | None:
        ...
        try:
            if task.task_kind == "prediction":
                events = await self._prediction_repository.list_prediction_events(
                    task.platform_job.id
                )
            else:
                events = await self._training_repository.list_events(
                    task.platform_job.id
                )
        except Exception:
            return None
    ...
    async def _derive(
        ...
        work_pool: dict[str, object] | None,
        logs: list[dict[str, object]],
    ) -> TaskTrackerDerived:
    ...
    def _scorecard(
        self, task: _TaskRecord, display_status: str, artifacts: list[dict[str, object]]
    ) -> TaskTrackerScorecard:
    ...
    def _rate_hint(self, summary: dict[str, object]) -> str | None:
        total = self._coerce_int(summary.get("total_samples"))
        processed = self._coerce_int(summary.get("processed"))
    ...
    def _console_lines(
        self, task: _TaskRecord, logs: list[dict[str, object]]
    ) -> list[str]:
        if logs:
            return [
                str(log.get("message", ""))
    ...
    def _summary_lines(self, summary: dict[str, object]) -> list[str]:
        lines = []
        for key in (
            "status",
            "processed",
    ...
    def _display_name(self, task: _TaskRecord) -> str:
        if task.task_kind == "training":
            return f"Training {task.platform_job.trainer_id}"
        if task.task_kind == "schedule_run":
    ...
    def _execution_kind(
        self, task: _TaskRecord, flow_run: dict[str, object] | None
    ) -> str:
        if task.task_kind == "training":
            return (
    ...
    def _execution_summary(
        self, task_runs: list[dict[str, object]], prefect_state: str | None
    ) -> str:
        if task_runs:
            completed = sum(
    ...
    def _task_run_label(self, task_run: dict[str, object], index: int) -> str:
    ...
    def _task_run_name(self, task_run: dict[str, object]) -> str | None:
    ...
    def _task_run_state_type(self, task_run: dict[str, object]) -> str | None:
        return self._nested_string(task_run, "state", "type") or self._string_or_none(
            task_run, "state_type"
        )

    def _task_run_status(self, task_run: dict[str, object], default_status: str) -> str:
    ...
    def _task_run_detail(self, task_run: dict[str, object]) -> str:
    ...
    def _nested_string(
        self, payload: dict[str, object] | None, *keys: str
    ) -> str | None:
        current: object = payload
        for key in keys:
            if not isinstance(current, dict):
