...
from app.modules.jobs.schedules.port.http.deps import get_scheduler_service
...
class MockSchedulerService:
    ...
    async def list_schedules_paginated(
        ...
        *,
        ...
        return [MOCK_DEPLOYMENT][offset : offset + limit], 1

    async def get_schedule(self, schedule_id: str, org_id=None) -> dict:
        return MOCK_DEPLOYMENT

    async def update_schedule(
        self, schedule_id: str, updates: dict, org_id: str
    ) -> dict:
    ...
    async def trigger_run(
        self, schedule_id: str, org_id: str, parameters=None
    ) -> dict:
        return MOCK_RUN

    async def pause_schedule(self, schedule_id: str, org_id: str) -> dict:
        return {**MOCK_DEPLOYMENT, "paused": True}

    async def resume_schedule(self, schedule_id: str, org_id: str) -> dict:
        return {**MOCK_DEPLOYMENT, "paused": False}

    async def list_runs(
        self, schedule_id: str, org_id: str, limit: int = 50
    ) -> list[dict]:
        return [MOCK_RUN]
