...
from app.modules.jobs.schedules.port.http.deps import SchedulerServiceDep
from app.modules.jobs.schedules.port.http.schemas import (
...
async def list_schedules(
    ...
    raws, total = await svc.list_schedules_paginated(
        org_id=org.id,
        offset=offset,
        limit=limit,
    )
    return PaginatedResponse(
        items=[_deployment_to_schedule(raw) for raw in raws],
        total=total,
    )
...
async def delete_schedule(
    ...
    await svc.delete_schedule(schedule_id, org.id)
...
async def trigger_run(
    ...
    raw = await svc.trigger_run(schedule_id, org.id)
...
async def pause_schedule(
    ...
    raw = await svc.pause_schedule(schedule_id, org.id)
    return _deployment_to_schedule(raw)
...
async def resume_schedule(
    ...
    raw = await svc.resume_schedule(schedule_id, org.id)
    return _deployment_to_schedule(raw)
...
async def list_runs(
    ...
    raws = await svc.list_runs(schedule_id, org.id, limit=limit)
...
async def get_run(
    ...
    raw = await svc.get_run(run_id, org.id)
    return _run_to_response(raw)
