...
def png_data_uri(size: int, red: int, green: int, blue: int) -> str:
    """Generate a PNG data URI for a solid-color square image.

    Constructs the PNG binary manually (no external dependencies).
    """
    raw_rows = bytearray()
...
def generate_synthetic_image(index: int, size: int = 224) -> str:
    """Generate a deterministic synthetic colored-square image.

    Uses ``random.Random(index)`` so the same index always produces
    the same image (useful for reproducible seeds).
    """
