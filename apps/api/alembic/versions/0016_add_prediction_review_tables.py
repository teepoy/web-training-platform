...
Revision ID: rev0016_pred_review
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_prediction_review_actions_dataset_id",
        "prediction_review_actions",
        ["dataset_id"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "review_action_id",
            sa.String(64),
            sa.ForeignKey("prediction_review_actions.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "annotation_id",
    ...
    op.create_index(
        "ix_annotation_versions_review_action_id",
        "annotation_versions",
        ["review_action_id"],
    )
    op.create_index(
        "ix_annotation_versions_annotation_id", "annotation_versions", ["annotation_id"]
    )


def downgrade() -> None:
    op.drop_index(
        "ix_annotation_versions_annotation_id", table_name="annotation_versions"
    )
    op.drop_index(
        "ix_annotation_versions_review_action_id", table_name="annotation_versions"
    )
