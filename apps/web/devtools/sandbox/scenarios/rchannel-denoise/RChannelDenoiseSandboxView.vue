<!-- Development-only seeded-dataset showcase. -->
<template>
    ...
    <n-spin :show="datasetLoading">
        ...
        >
          <template #footer>
            <n-space vertical align="center">
              ...
              <n-text depth="3">
                Run <n-code>make ensure-sandbox-datasets</n-code> from the repo root to seed this
                dataset.
              </n-text>
            </n-space>
          </template>
      ...
      <template v-else-if="datasetId">
        <n-space vertical>
          <n-space align="center" justify="space-between">
            <n-space align="center">
              ...
              <n-tag type="info" size="medium" round> {{ totalCount }} samples </n-tag>
            </n-space>
            <n-space>
              <n-button
                size="small"
          ...
          <n-text depth="3" style="max-width: 720px">
            5 R-channel grayscale images per sample: noisy 32×32, clean 32×32, difference 32×32,
            full 224×224, random crop 224×224. First 100 ImageNet classes, salt & pepper noise (5%).
          </n-text>
        ...
        <n-drawer v-model:show="drawerVisible" :width="640">
          <n-drawer-content
            v-if="selectedSample"
            :title="`Sample: ${selectedSample.id.slice(0, 8)}...`"
          >
...
const dataset = computed(() => datasetQuery.data.value?.find((d) => d.name === DATASET_NAME));
...
// ---------------------------------------------------------------------------
const browserItems = computed<BrowserItem[]>(() => samples.value.map(mapSampleToBrowserItem));
