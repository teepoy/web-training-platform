...
test("wafer-map panel renders with dieGrid config @legacy", async ({ authedPage, apiMocks }) => {
...
test("wafer-map zoom + Reset Zoom button @legacy", async ({ authedPage, apiMocks }) => {
  const page = new WaferMapPage(authedPage);
  ...
  await authedPage.evaluate(() => new Promise((resolve) => requestAnimationFrame(resolve)));
  await authedPage.evaluate(() => new Promise((resolve) => requestAnimationFrame(resolve)));
