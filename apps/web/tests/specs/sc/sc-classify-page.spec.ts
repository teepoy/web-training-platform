...
test('SC classify page renders blink table and annotation sidebar @mock', async ({
  authedPage,
  apiMocks,
}) => {
  ...
  await apiMocks.datasets.mockListSamples(
    datasetId,
    makeSamples(datasetId, 2),
  )
  ...
  await expect(
    authedPage.getByText('SC Wafer Dataset'),
...
test('SC classify page shows error state for missing dataset @mock', async ({
  authedPage,
}) => {
