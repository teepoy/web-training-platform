...
export async function mockCreatePreviewSession(
  ...
  await page.route('**/api/v1/preview-sessions', async (route) => {
    if (route.request().method() === 'POST') {
      const body: PreviewSessionResponse = {
  ...
  await page.route(`**/api/v1/preview-sessions/${sessionId}`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        next_cursor: 'cursor-20',
        has_more: true,
      } satisfies PreviewSessionResponse),
    })
  })
}
...
export async function mockGetPreviewItems(
  ...
  const defaultItems: PreviewItemResponse[] = items ?? [
    {
  ...
  const body: PreviewItemsResponse = {
    items: defaultItems,
    next_cursor: null,
    has_more: false,
...
export async function mockPersistPreview(
  ...
  await page.route(
    `**/api/v1/preview-sessions/${sessionId}/persist`,
    async (route) => {
      const body: PersistStatusResponse = {
...
export async function mockGetPersistStatus(
  ...
  await page.route(
    `**/api/v1/preview-sessions/${sessionId}/persist-status`,
    async (route) => {
      const body: PersistStatusResponse = {
        dataset_id: datasetId,
        persist_session_id: 'persist-123',
...
export async function mockExpiredPreviewSession(
  page: Page,
  sessionId: string,
): Promise<void> {
  await page.route(
    `**/api/v1/preview-sessions/${sessionId}`,
    async (route) => {
      await route.fulfill({
        status: 404,
        contentType: 'application/json',
