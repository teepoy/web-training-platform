...
export async function mockAnnotationStats(
  ...
  await page.route(
    `**/api/v1/datasets/${datasetId}/annotation-stats`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
export async function mockGetSample(
  ...
  await page.route(
    `**/api/v1/datasets/${datasetId}/samples/${sampleId}`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
export async function mockSampleSimilar(
  ...
  await page.route(
    `**/api/v1/datasets/${datasetId}/samples/${sampleId}/similar`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
...
export async function mockDatasetQuery(
  page: Page,
  datasetId: string,
): Promise<void> {
