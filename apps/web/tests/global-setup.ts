...
async function globalSetup(config: FullConfig): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const loginResp = await new Promise<{ status: number; body: { access_token: string; user: unknown } }>(
        (resolve, reject) => {
          ...
          const req = request(
            {
              ...
              method: 'POST',
              headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(loginBody).toString() },
            },
            (res) => {
              let raw = ''
      ...
      console.warn('[global-setup] Backend unreachable — skipping storage state (mock-only or backend down).')
      return
    }
  }
