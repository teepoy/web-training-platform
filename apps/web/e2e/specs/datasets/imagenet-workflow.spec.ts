...
test("navigate to seeded ImageNet dataset @live @slow", async ({ page, liveAuth }) => {
  ...
  await expect(page.getByText(SEEDED_DATASET, { exact: true }).first()).toBeVisible({
    timeout: 10_000,
  });
  ...
  await expect(page.getByRole("button", { name: "Open Workflow" })).toBeVisible({ timeout: 5_000 });
...
test("open workflow and launch predictions @live @slow", async ({ page, liveAuth }) => {
  ...
  const modelSelect = predictionCard
