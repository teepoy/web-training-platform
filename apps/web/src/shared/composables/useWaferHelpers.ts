 ...
 *
 * Shared by dataset and SC workbench surfaces so every consumer applies the
 ...
 *
 * Collection keys vary per surface; dataset browsing uses "browser-items".
 */
export function injectWaferPanelData(
  ...
  return panels.map((panel) => {
    ...
    const config = (panel.props.config as Record<string, unknown> | undefined) ?? {};
