...
export const SIDEBAR_WIDGET_INTERACTION_KEY: InjectionKey<ComputedRef<SidebarWidgetInteractionContext>> =
...
export function defineSidebarWidget(definition: SidebarWidgetDefinition): SidebarWidgetDefinition {
  return definition
}
...
export function runSidebarWidgetSelfTest(
  ...
  const checks: SidebarWidgetSelfTestCheck[] = [
    ...
    },
    {
      ...
      detail: 'Display name must be present for authors and registry consumers.',
    },
    {
      name: 'description',
    ...
    },
    {
      ...
      detail: 'Every accepted prop must be listed explicitly when the widget accepts props.',
    },
    {
      name: 'context keys',
      passed: contract.capabilities.reads.every(key => VALID_CONTEXT_KEYS.includes(key)),
      detail: 'Read contexts must come from the shared sidebar context vocabulary.',
    },
    {
      name: 'intent types',
      passed: contract.capabilities.emits.every(intent => VALID_INTENT_TYPES.includes(intent)),
      detail: 'Emitted intents must stay inside the typed interaction vocabulary.',
    },
    {
      name: 'self-test scenarios',
      passed: contract.selfTests.length > 0,
      detail: 'Every widget needs at least one author-facing self-test scenario.',
    },
    {
      name: 'scenario completeness',
      passed: contract.selfTests.every(scenario => (
        ...
        && scenario.expected.every(expectation => expectation.trim().length > 0)
      )),
      detail: 'Each self-test scenario needs a name, objective, steps, and expected outcomes.',
    },
    {
      name: 'interactive widgets declare interaction-state',
      passed: contract.capabilities.emits.length === 0
        || contract.capabilities.reads.includes('interaction-state'),
      detail: 'Widgets that emit intents should also read from interaction-state.',
    },
  ]
