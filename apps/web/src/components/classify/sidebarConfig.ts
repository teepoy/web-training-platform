...
export const SIDEBAR_WIDGETS: Record<string, SidebarWidgetDefinition> = {
  ...
  "label-distribution": defineSidebarWidget({
    ...
    contract: {
      ...
      description:
        "Displays label counts and now supports click-to-filter behavior for the classify surface.",
  ...
  "wafer-map": defineSidebarWidget({
    key: "wafer-map",
    component: defineAsyncComponent(() => import("./widgets/WaferMapWidget.vue")),
    contract: {
      displayName: "Wafer Map",
      description:
        "Renders high-density wafer scatter points and can emit linked selection/filter intents.",
      acceptsProps: ["data", "config", "size"],
      capabilities: {
        reads: ["interaction-state"],
        emits: ["select-samples", "select-predictions", "apply-filter", "clear-selection"],
...
export const WIDGET_COMPONENTS: Record<string, Component> = Object.fromEntries(
  Object.entries(SIDEBAR_WIDGETS).map(([key, definition]) => [
    key,
