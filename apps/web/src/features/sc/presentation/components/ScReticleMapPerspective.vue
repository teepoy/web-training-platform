...
onUpdated(() => console.debug("[render] ScReticleMapPerspective"));

function drawOverlay() {
  console.debug("[map] ScReticleMap drawOverlay");
  ...
  if (props.highlightDefects && props.highlightDefects.length > 0) {
    ...
    for (const hd of props.highlightDefects) {
      const [cx, cy] = dataToScreen(transform, hd.reticleX, hd.reticleY);
      ctx.moveTo(cx - 3, cy);
      ctx.lineTo(cx + 3, cy);
      ctx.moveTo(cx, cy - 3);
      ctx.lineTo(cx, cy + 3);
...
watch(
  () => props.points,
  () => {
    console.debug("[map] ScReticleMap watch(points) → redraw");
    recalcTransform();
    drawOverlay();
  },
);
