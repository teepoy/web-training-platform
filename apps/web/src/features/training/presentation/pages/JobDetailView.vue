...
const metricsArtifactRef = computed(
  () =>
    job.value?.artifact_refs?.find((artifact: { kind: string }) => artifact.kind === "metrics") ??
    null,
);
...
const { data: metricsArtifact } = useQuery({
  queryKey: computed(() =>
    orgScopedQueryKey(orgStore.currentOrgId, [
      "job-metrics-artifact",
      id.value,
      metricsArtifactRef.value?.id ?? null,
    ]),
  ),
  queryFn: async () => {
    ...
    const artifactData = await downloadArtifactApiV1ArtifactsArtifactIdDownloadGet(artifact.id);
    return artifactData as Record<string, unknown> | null;
  },
