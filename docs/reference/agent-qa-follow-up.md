...
| AQA-006 | Replaced the legacy `wafer-demo` step with the current sparse SC inspection import. | Historical verification reused the same 2,500-row inspection, dataset, and ready collection revision. The former `seed-dev` entry point has since been removed. | Smoke-test Reclassify against the imported `source_inspection_time` and `source_wafer_key`; keep the 300k Sample Table benchmark separate.  |
