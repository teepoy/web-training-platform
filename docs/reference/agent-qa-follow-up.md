...
Status: Open (AQA-001 and AQA-005 verified fixed)
Recorded: 2026-08-01; updated: 2026-08-03
...
| AQA-006 | `make seed-dev` still creates the legacy `db_full` `Wafer Demo`.          | The 2026-08-03 rebuild produced a dataset without `dataset_meta.source_inspection_time` or `source_wafer_key`; Reclassify stopped at `Inspection metadata not available for this dataset`.          | Replace the final legacy `wafer-demo` step with the current SC inspection import path, or retire it from `seed-dev`; smoke-test that the resulting dataset mounts a 300k Sample Table. |
