...
- Cache misses use process-local singleflight plus a filesystem advisory lock. The lock holder rechecks the final target, streams into a sibling temporary location, validates and syncs it, then publishes with an atomic rename. Janitor deletion coordinates with the same lock.
