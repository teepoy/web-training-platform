...
- Contexts close explicitly, on stream cancellation, or after an idle timeout; long-running streams have no fixed total lifetime. An opening acknowledgement advertises batch, response-byte, active-context, and bounded in-flight-request limits. Prediction uses a two-batch prefill window so the service can parse the next batch while the client preprocesses or models the current result; Training and Export initially use one. This window does not permit unbounded buffering or out-of-order responses. Image payloads are not recompressed by gRPC.
...
- The blocking performance gate uses 300,000 representative samples with two compressed images per sample and measures warm-cache service parsing through Python receipt while filling the advertised request window. It must sustain at least 3,000 samples per second. Cold-cache source throughput and an additional decode/fake-predictor run are reported separately rather than folded into that gate.
