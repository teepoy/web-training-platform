...
Manual linking and rule-driven discovery both produce the same underlying relationship: a Dataset becomes a member of a Collection. Making Manual Collection and Dynamic Collection separate resource types would duplicate permissions, Snapshot publication, training, prediction, and management surfaces even though only the source of membership differs.
...
- An already imported Source record that later changes upstream is marked `Source changed`; it is not synchronized in place. A compatible Re-import publishes a new Revision audit record for that Automation-owned Dataset; an incompatible output contract creates a new Dataset. The Collection can record the new Dataset Revision in a later Snapshot while retaining historical audit provenance.
...
- One discovery run processes a batch of Source records and publishes at most one Snapshot containing all successfully admitted members. Individual failures are retained with visible errors and can be retried later without blocking successful admissions.
...
- Dataset and Snapshot provenance must retain upstream identity and Source version so the platform can detect a changed Source record without content hashing.
...
- A partially successful discovery run is visible as partial rather than being reported as fully successful; its published Snapshot contains only members whose admission completed.
- The current architecture statement that automated refresh never changes membership must be revised when implementation begins; membership admission and Snapshot publication remain separate audited steps.
