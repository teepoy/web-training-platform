...
- Backfill, bulk prediction reconciliation, and candidate training require an impact summary and explicit confirmation before submission. The summary identifies the target Collection, pinned rule/Model/Snapshot inputs, and the estimated child work rather than relying on administrator approval.
- If the target scope, pinned rule/Model/Snapshot, or estimated child-task count cannot be resolved, submission is blocked with an explicit reason. A compute-cost estimate may remain unavailable as long as the concrete work scope is known.
...
- Dataset Revision notifications list affected Collections. Users normally review and refresh one Collection at a time, while the notification surface may multi-select Collections, show the combined impact, and submit independent refreshes without an update-all shortcut.
