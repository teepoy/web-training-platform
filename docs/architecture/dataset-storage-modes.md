...
**Train.** `train_job` opens the dataset through `DatasetStorageFactory`, then calls `storage.list_samples(with_labels=True, return_lazyframe=True)`. External runtimes consume the data-plane manifest. Each API-local algorithm module owns materialization and its temporary workspace. The SC ML helpers accept the resulting Parquet paths and expose plain inspection/iterator functions. ResNet reopens the iterator per epoch; Ultralytics consumes it once to build its local `train/<compact-class>/` folder. The checkpoint remains a local path until `ArtifactOutput` consumption uploads it. No shared ResNet/Ultralytics runtime context, legacy `RuntimeMaterializer`, `BulkViewLoader`, or `DatasetSampleService` is used.

**Predict.** `predict_job` opens the dataset through `DatasetStorageFactory`, then calls `storage.list_samples(return_lazyframe=True, sample_ids=...)`. Each algorithm module materializes bounded Parquet, downloads its checkpoint to its own temporary directory, and streams `iter_sc_prediction_samples(...)` directly into its model implementation. Only the storage write path is shared. Predicting the full dataset should pass `sample_ids=None`; sending 100k IDs through Prefect parameters exceeds Prefect's serialized-parameter limit.
