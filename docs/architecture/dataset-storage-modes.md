...
**Train.** `train_job` opens the dataset through `DatasetStorageFactory`, then calls `storage.list_samples(with_labels=True, return_lazyframe=True)`. External runtimes consume the data-plane manifest. API-local compatibility trainers use the registered SC view materializer, which writes temporary Parquet in bounded batches. `ScTrainingDataset` then reads row groups lazily with bounded-memory shuffle instead of collecting all image bytes into a Python list. ResNet replays that dataset per epoch; YOLO consumes it once to build its required local `train/<compact-class>/` folder inside the same owned workspace. The checkpoint remains a local path until `ArtifactOutput` consumption uploads it. No legacy `RuntimeMaterializer`, `BulkViewLoader`, or `DatasetSampleService` is used.

**Predict.** `predict_job` opens the dataset through `DatasetStorageFactory`, then calls `storage.list_samples(return_lazyframe=True, sample_ids=...)`. The API-local compatibility predictor owns a prediction workspace: the registered SC materializer writes bounded Parquet batches, `ScPredictionDataset` reads row groups lazily without collecting the full table, and object storage downloads the model directly to a local checkpoint path. ResNet and YOLO consume that path and stream prediction samples into the existing batched `write_predictions` path. Predicting the full dataset should pass `sample_ids=None`; sending 100k IDs through Prefect parameters exceeds Prefect's serialized-parameter limit.
