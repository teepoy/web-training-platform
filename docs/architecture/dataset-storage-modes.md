...
| Export (with predictions)                    | Supported | Supported           | Generic `sparse-export-v1` JSON remains available. SC sparse Datasets also expose Annotation, Prediction, or Final Class KLARF 1.2/1.8, Parquet, and ZIP package exports with optional server-side Review Sampling.                                                                                               |
...
**Import.** SC Import as Dataset uses the API service direct sparse import path. It reads upstream defect rows, writes only `sample_id` and `defect_id`, records inspection identity in Dataset metadata, and publishes the sparse manifest / locator index. It does not materialize source attributes or image bytes during import and no longer registers a Prefect `sc_import` deployment.
...
### SC source schema v4 and earlier compatibility
...
**Schema version gating.** `DatasetManifest.schema_version` distinguishes
`v4_identity`, v3, v2, and legacy source rows. Current imports carry
`schema_version="v4_identity"`; v3 retains copied scalar upstream fields, v2
retains the embedded `images` list, and legacy datasets have `None` or `"v1"`.

V2/v3 remain explicit read-compatibility formats. Source-dependent SC readers
project `sample_id` and `defect_id`, ignore copied source extras, and enrich the
membership from current upstream data. The v2 image endpoint may still read the
embedded list for its dedicated compatibility contract. Legacy URI-only
behavior remains limited to legacy-aware readers and is not a source-data
fallback for a versioned manifest.
...
- Imported SC dataset views and preview both use scalar Inspection identity through `/api/v1/sc/images/...`, forwarded by the web gateway directly to image-parser. FastAPI has no SC image-bytes route or fallback. Browser adapters append authentication before rendering.
