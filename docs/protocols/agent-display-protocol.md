...
2. **Data source reference** — Points to an API endpoint or Vue context:
   ```json
   {
     "data_source": {
       "kind": "api",
       "endpoint": "/datasets/{id}/query",
       "params": { "query_type": "annotation-stats" }
     }
   }
   ```
...
5. Tool calls are executed with the required service ports (repository, training/prediction submission, scheduler service, etc.)
