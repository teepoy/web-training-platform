/**
 * @platform/plugin-sdk — Preview launcher plugin contracts
 *
...
export function definePreviewPlugin(
  descriptor: PreviewLauncherDescriptor,
): PreviewLauncherDescriptor {
  if (!descriptor.id || descriptor.id.trim() === "") {
    throw new Error("[plugin-sdk] definePreviewPlugin: id must not be empty");
  }
  if (!descriptor.label || descriptor.label.trim() === "") {
    throw new Error(
      `[plugin-sdk] definePreviewPlugin(${descriptor.id}): label must not be empty`,
    );
  }
  if (!descriptor.component) {
    throw new Error(
      `[plugin-sdk] definePreviewPlugin(${descriptor.id}): component must be provided`,
    );
  }
  if (descriptor.surfaces.length === 0) {
    throw new Error(
      `[plugin-sdk] definePreviewPlugin(${descriptor.id}): surfaces must not be empty`,
    );
  }
  return descriptor;
}