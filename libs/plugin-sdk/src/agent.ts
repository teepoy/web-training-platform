/**
 * @platform/plugin-sdk — Agent skill plugin contracts
 *
...
export function defineAgentSkill(
  ...
  if (!descriptor.toolName || descriptor.toolName.trim() === "") {
    throw new Error("[plugin-sdk] defineAgentSkill: toolName must not be empty");
  }
  if (!descriptor.displayName || descriptor.displayName.trim() === "") {
    throw new Error(
      `[plugin-sdk] defineAgentSkill(${descriptor.toolName}): displayName must not be empty`,
    );
  }
  if (descriptor.surfaces.length === 0) {
    throw new Error(
      `[plugin-sdk] defineAgentSkill(${descriptor.toolName}): surfaces must not be empty`,
    );
  }
  return descriptor;
}
