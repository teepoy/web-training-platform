...
describe("defineSidebarPlugin", () => {
  it("returns the descriptor unchanged for a valid input", () => {
    const d = defineSidebarPlugin({
      key: "my-widget",
...
describe("defineImportPlugin", () => {
  it("validates successfully", () => {
    const d = defineImportPlugin({
      id: "csv-importer",
      label: "CSV",
      surfaces: ["dataset"],
      component: StubComponent,
...
describe("defineExportPlugin", () => {
  it("validates successfully", () => {
    const d = defineExportPlugin({
      id: "csv-exporter",
      label: "CSV Export",
      surfaces: ["dataset"],
      component: StubComponent,
...
describe("createPluginRegistry", () => {
  ...
  it("registers and retrieves sidebar widgets", () => {
    ...
    registry.registerSidebarWidget(makeWidget("widget-a"));
    expect(registry.getSidebarWidget("widget-a")?.key).toBe("widget-a");
    expect(registry.getSidebarComponent("widget-a")).toBe(StubComponent);
    expect(registry.getSidebarComponent("missing")).toBeUndefined();
  });

  it("warns and overwrites duplicate sidebar widget", () => {
    const registry = createPluginRegistry();
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    registry.registerSidebarWidget(makeWidget("dup"));
    registry.registerSidebarWidget(makeWidget("dup"));
