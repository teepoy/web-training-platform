...
def register_seed_user(client: httpx.Client, email: str, password: str, name: str) -> httpx.Response:
    return api_request(
        client,
        "post",
...
def promote_superadmin(compose_file: str, email: str, password: str, name: str) -> None:
    cmd = [
        ...
        f"--email={email}", f"--password={password}", f"--name={name}",
    ]
    ...
    if result.returncode != 0:
        ...
        print(f"  If running locally, use: make create-superadmin EMAIL={email} PASSWORD={password} NAME='{name}'")
    else:
        print(f"  {result.stdout.strip()}")


def resolve_or_create_org(client: httpx.Client, org_name: str, org_slug: str) -> str | None:
