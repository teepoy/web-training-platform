...
def _items_from_collection_response(body: Any) -> Iterable[dict[str, Any]]:
    """Normalize old list responses and new paginated API responses."""
    raw_items = body.get("items", body) if isinstance(body, dict) else body
...
def _find_by_name(items: Any, name: str) -> dict[str, Any] | None:
    for item in _items_from_collection_response(items):
